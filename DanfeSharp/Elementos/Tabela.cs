...
namespace DanfeSharp.Elementos
{
    internal class Tabela : ElementoBase
    {
        ...
        public float AlturaTabela(PointF p, float width)
        {
            ...
            #region Faz o cálculo do tamanho das linhas dos produtos

            float x = p.X;
            float y = p.Y;

            int count = 0;
            while (count < Linhas.Count)
            {
                TextBlock[] tb = new TextBlock[Colunas.Count];
                ...
                y += Math.Max(tbm, FonteCorpo.AlturaLinha) + PaddingSuperior + PaddingInferior;

                ac += Math.Max(tbm, FonteCorpo.AlturaLinha) + PaddingSuperior + PaddingInferior;
                count++;
            }

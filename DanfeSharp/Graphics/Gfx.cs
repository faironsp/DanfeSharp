...
namespace DanfeSharp.Graphics
{
    internal class Gfx
    {
        ...
        public void ShowText(String text, PointF point)
        {
            ...
            try
            {
                char charEspacoCorretoCode = (char)32;

                List<char> lWrongChar = new List<char>();
                
                lWrongChar.Add((char)160);
                lWrongChar.Add((char)129);

                string lNewText = string.Empty;
                foreach (var item in lWrongChar)
                {
                    text = text.Replace(item, charEspacoCorretoCode);
                }
            }
            catch { }
